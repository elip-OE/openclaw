...
const rateLimiter = createFixedWindowRateLimiter({ maxRequests: 30, windowMs: 60_000, maxTrackedKeys: 5_000 });
...
function rememberWebhookMessage(params: { accountId: string; messageSid: string; now?: number }): boolean {
...
export function createSmsWebhookHandler(params: SmsWebhookHandlerParams) {
  return async (req: IncomingMessage, res: ServerResponse) => {
    if (req.method !== "POST") { respondTwiml(res, 405, "Method not allowed"); return true; }
    ...
    try { form = await readTwilioWebhookForm(req); } catch { respondTwiml(res, 400, "Invalid request body"); return true; }

    if (!params.account.dangerouslyDisableSignatureValidation) {
      const ok = verifyTwilioSignature({
        signature: headerValue(req.headers["x-twilio-signature"]),
        url: resolveTwilioWebhookSignatureUrl({ req, publicWebhookUrl: params.account.publicWebhookUrl }),
        authToken: params.account.authToken,
        form,
      });
      if (!ok) { params.log?.warn?.("SMS webhook rejected invalid Twilio signature"); respondTwiml(res, 403, "Invalid signature"); return true; }
    }

    const msg = buildTwilioInboundMessage(form);
    if (!msg) { respondTwiml(res, 400, "Missing SMS payload"); return true; }
    ...
    if (!rememberWebhookMessage({ accountId: params.account.accountId, messageSid: msg.messageSid })) {
    ...
    void dispatchSmsInboundEvent({
      cfg: params.cfg, account: params.account, msg,
      channelRuntime: params.channelRuntime, log: params.log,
    }).catch((err: unknown) => {
