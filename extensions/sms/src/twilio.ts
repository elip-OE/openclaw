// Twilio API client for SMS plugin.
...
type TwilioApiResponse = { ok: boolean; status: number; text: string };
type TwilioMessagePayload = { sid?: string; to?: string; from?: string; status?: string };

export type TwilioIncomingPhoneNumber = {
  sid: string; phoneNumber: string; smsUrl: string; smsMethod: string; voiceUrl: string;
};
export type TwilioMessageLogEntry = {
  sid: string; direction: string; status: string; to: string; from: string;
  errorCode: string; body: string; dateCreated: string; dateSent: string;
};
...
function firstTrimmedString(value: unknown): string { return firstString(value).trim(); }
...
function parseTwilioApiError(text: string): { code?: number; message?: string } {
  try {
...
function requestSearch(req: IncomingMessage): string {
  try { return new URL(req.url ?? "/", "http://localhost").search; } catch { return ""; }
}
...
export function resolveTwilioWebhookSignatureUrl(params: { req: IncomingMessage; publicWebhookUrl: string }): string {
...
export class TwilioSmsApiError extends Error {
  ...
  constructor(httpStatus: number, responseText: string, operation = "send") {
    ...
    super(`Twilio SMS ${operation} failed (${httpStatus}): ${parsed.message ?? (responseText || "unknown")}`);
...
export function computeTwilioSignature(params: { url: string; authToken: string; form: Record<string, string> }): string {
...
export function verifyTwilioSignature(params: { signature: string | undefined; url: string; authToken: string; form: Record<string, string> }): boolean {
  if (!params.signature || !params.url || !params.authToken) return false;
  return safeEqual(params.signature, computeTwilioSignature({ url: params.url, authToken: params.authToken, form: params.form }));
}
...
export async function readTwilioWebhookForm(req: IncomingMessage): Promise<Record<string, string>> {
  const body = await readRequestBodyWithLimit(req, { maxBytes: WEBHOOK_BODY_LIMIT_BYTES, timeoutMs: WEBHOOK_BODY_TIMEOUT_MS });
...
async function requestTwilioApi(params: {
  url: string; account: ResolvedSmsAccount; init?: RequestInit; timeoutMs?: number;
}): Promise<TwilioApiResponse> {
  const init = { ...params.init, headers: { ...normalizeHeaders(params.init?.headers), authorization: basicAuthHeader(params.account) } };
  const guarded = await fetchWithSsrFGuard({ url: params.url, init, auditContext: "sms-twilio-api", policy: { allowedHostnames: [TWILIO_API_HOSTNAME] }, requireHttps: true, timeoutMs: params.timeoutMs ?? TWILIO_API_TIMEOUT_MS });
  try {
    return { ok: guarded.response.ok, status: guarded.response.status, text: await guarded.response.text() };
  } finally { await guarded.release(); }
}
...
function parseTwilioIncomingPhoneNumber(record: Record<string, unknown>): TwilioIncomingPhoneNumber {
  return {
...
function parseTwilioListPayload<T>(text: string, key: string, parseEntry: (r: Record<string, unknown>) => T): T[] {
  if (!text.trim()) return [];
  const parsed = JSON.parse(text) as Record<string, unknown>;
  if (!parsed || typeof parsed !== "object") return [];
  const items = parsed[key];
  if (!Array.isArray(items)) return [];
  return items.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === "object" && !Array.isArray(item))).map(parseEntry);
}

export async function listTwilioIncomingPhoneNumbers(params: { account: ResolvedSmsAccount; phoneNumber?: string; timeoutMs?: number }): Promise<TwilioIncomingPhoneNumber[]> {
  const query = new URLSearchParams();
  if (params.phoneNumber) query.set("PhoneNumber", params.phoneNumber);
  const response = await requestTwilioApi({ account: params.account, url: twilioApiUrl(params.account.accountSid, "/IncomingPhoneNumbers.json", query), timeoutMs: params.timeoutMs });
...
export async function listTwilioMessages(params: { account: ResolvedSmsAccount; to?: string; from?: string; pageSize?: number; timeoutMs?: number }): Promise<TwilioMessageLogEntry[]> {
  const query = new URLSearchParams();
  if (params.to) query.set("To", params.to);
  if (params.from) query.set("From", params.from);
  query.set("PageSize", String(params.pageSize ?? 5));
  const response = await requestTwilioApi({ account: params.account, url: twilioApiUrl(params.account.accountSid, "/Messages.json", query), timeoutMs: params.timeoutMs });
...
export async function sendSmsViaTwilio(params: { account: ResolvedSmsAccount; to: string; text: string }): Promise<SmsSendResult> {
  ...
  const body = new URLSearchParams({ To: params.to, Body: params.text });
