...
import type { AwsSmsChannelRuntime } from "./inbound.js";
import {
...
function createAwsSmsReceipt(params: {
  results: Array<{ messageId: string; to: string; from?: string }>;
  kind: "text" | "media";
}) {
...
async function sendAwsSmsText(ctx: {
  ...
  return createAwsSmsReceipt({ results, kind: "text" });
}
      ...
      gateway: {
        startAccount: async (ctx) => {
          if (!ctx.channelRuntime) {
            ctx.log?.warn?.("AWS SMS channel runtime is not available; webhook route not started");
            return;
          }
          return await startAwsSmsGatewayAccount({
            cfg: ctx.cfg,
            account: ctx.account,
            channelRuntime: ctx.channelRuntime as unknown as AwsSmsChannelRuntime,
